...
func main() {
	//Agrupamos en una variable de tipo string la información requeida
	psqlInfo := fmt.Sprintf("user=%s password=%s host=%s port=%d " +
													"dbname=%s sslmode=%s",
													user, password, host, port,  dbname, sslmod)

//Se abre la conexión a la BD
	db, error := sql.Open("postgres", psqlInfo)
fmt.Println("Variable error: ", error)
	//evaluo el resultado de la llamada
	if error != nil {//De contener algo la variable hay un error
		fmt.Println("Error de acceso")
	} else {//si no contiene nada es por que la conexión fue exitosa
